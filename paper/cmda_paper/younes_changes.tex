...
\author{ \href{https://orcid.org/0000-0000-0000-0000}{\includegraphics[scale=0.06]{orcid.pdf}\hspace{1mm}Pejman Farhadi Ghalati}\thanks{Use footnote for providing further
	...
	\And
	\href{https://orcid.org/0000-0000-0000-0000}{\includegraphics[scale=0.06]{orcid.pdf}\hspace{1mm}Younes Mueller} \\
    Joint Research Center for Computational Biomedicine\\
	RWTH Aachen University\\
	Aachen, Germany \\
	\texttt{email@combine.rwth-aachen.de} \\
	\And
	\href{https://orcid.org/0000-0000-0000-0000}{\includegraphics[scale=0.06]{orcid.pdf}\hspace{1mm}Andreas Schuppert} \\
    Joint Research Center for Computational Biomedicine\\
	RWTH Aachen University\\
	Aachen, Germany \\
	\texttt{email@combine.rwth-aachen.de} \\
...
Generally, feature extraction or engineering represents the masked characteristics of raw temporal data\todo{Don't understand the sentence. Maybe: Genrally, feature extraction or feature engineering tries to extract the masked characteristics of raw temporal data.}. In other words, it can be described as a set of features corresponding to the signal's specific form or pattern. Additionally, feature extraction is generally a procedure that reduces the dimensionality of an input signal or compresses its data and thus reduces the number of resources required to analyze it. 
...
Users can select from a number of built-in filters, through a dynamic attribute of the \codeword{Filters} object, called \codeword{.add} that gives the user access to the following functions:
\begin{longtable}
{ |c||c|p{8 cm}|}
%  \hline
...
It provides several ready-to-use built-in features of time domain, spectral domain, nonlinear, and wavelet  that can be selected and added to a \codeword{Features} object instance via \codeword{.add} function.\\
...
\codeword{cmda.feature_extraction.Features} is a tool for extracting multiple features at the same time from a numeric signal \codeword{x}. One can select from a wide variety of built-in features, as well as adding user-defined features. This functionality makes it possible to tailor the features to the need of the user.
